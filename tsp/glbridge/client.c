...
extern void glOrthof(GLfloat l, GLfloat r, GLfloat b, GLfloat t, GLfloat n,
                     GLfloat f);
...
#define attribs (tspgl_shared()->attribs)
#define gles2_mvp (tspgl_shared()->gles2_mvp)
#define gles2_mvp_valid (tspgl_shared()->gles2_mvp_valid)
...
/* Native GLES treats a pointer as a VBO offset only if ARRAY_BUFFER is bound.
 * IronMonkey/the EapRules loader often leave a VBO bound and still pass a
 * client (stack/heap) pointer. On Mali that still works; the 32→64 bridge
 * must copy those bytes. Userspace pointers on armhf bionic are >> 64K. */
static int ptr_is_vbo_offset(const void *ptr)
{
                           ...
                           int stride, const void *ptr)
{
    ...
    attribs[index].is_offset = ptr_is_vbo_offset(ptr);
    u[0] = index;
    u[1] = (uint32_t)size;
    u[2] = (type == 0x140C) ? GL_FLOAT : type; /* GL_FIXED -> float */
    u[3] = (uint32_t)normalized;
    u[4] = (uint32_t)stride;
...
void tspgl_es1_enable_array(unsigned index, int on)
{
    ...
    if (index >= 16)
        return;
    attribs[index].enabled = on;
    tspgl_call(on ? OP_glEnableVertexAttribArray : OP_glDisableVertexAttribArray,
               &u, 4, NULL, 0);
}
...
#define glEnable glEnable_gen
#define glDisable glDisable_gen
#define glIsEnabled glIsEnabled_gen
#define glClear glClear_gen
...
{
    if (tspgl_es1_set_enable(cap, 0))
        return;
    glDisable_gen(cap);
}

uint32_t glIsEnabled(GLenum cap)
{
    int v = tspgl_es1_is_enabled(cap);
    if (v >= 0)
        return (uint32_t)v;
    return glIsEnabled_gen(cap);
}
...
void glActiveTexture(GLenum texture)
{
    ...
    tspgl_es1_set_active_tex(texture);
}
...
void glBindFramebuffer(GLenum target, GLuint fb)
{
    ...
    if (target == 0x8D40) {
        ...
        bound_read_fb = fb;
    if (nlog < 8) {
        fprintf(stderr, "tspgl: BindFB target=0x%x fb=%u\n", target, fb);
        fflush(stderr);
        nlog++;
    }
...
void glClear(GLbitfield mask)
{
    static int nlog;
    if (nlog < 8) {
        fprintf(stderr, "tspgl: Clear mask=0x%x fb=%u vp=%dx%d\n",
                (unsigned)mask, bound_fb, (int)st_viewport[2],
                (int)st_viewport[3]);
        fflush(stderr);
        nlog++;
    }
...
{
    static int nlog;
    /* 32-bit SDL offscreen on TSP often reports 0x0 or the 1280x720 panel.
     * The presenter FBO is 640x480; a 0x0 viewport draws nothing (black
     * screen, audio still runs). Do not clamp 640x960 portrait FBOs. */
    if (w <= 0 || h <= 0 || w >= 1280 || h >= 720) {
        fprintf(stderr, "tspgl: viewport %d,%d %dx%d -> 0,0 640x480\n",
        ...
        fprintf(stderr, "tspgl: viewport %d,%d %dx%d\n", (int)x, (int)y,
                (int)w, (int)h);
        fflush(stderr);
        nlog++;
    }
...
const GLubyte *glGetString(GLenum name)
{
    ...
    if (name == 0x1F02) { /* GL_VERSION — Dead Space TSP client string */
        memcpy(version, "OpenGL ES-CM 1.1", 17);
        return (const GLubyte *)version;
    }
    if (name == 0x1F03) { /* GL_EXTENSIONS */
        static const char *es1_ext =
            ...
            "GL_OES_blend_equation_separate GL_OES_stencil8 "
            "GL_OES_mapbuffer GL_OES_matrix_palette GL_OES_draw_texture "
            "GL_OES_point_sprite GL_OES_point_size_array "
            "GL_EXT_texture_format_BGRA8888 GL_AMD_compressed_ATC_texture";
        memset(ext, 0, sizeof(ext));
        memcpy(ext, es1_ext, strlen(es1_ext));
...
void glGetIntegerv(GLenum pname, GLint *params)
{
    ...
    if (tspgl_es1_get_integerv(pname, params))
        return;
    if (logged < 10) {
...
void glGetFloatv(GLenum pname, GLfloat *params)
{
    ...
    int32_t iv[4];
    memset(out, 0, sizeof(out));
    if (tspgl_es1_get_floatv(pname, params))
        return;
...
void glGetBooleanv(GLenum pname, GLboolean *params)
{
    ...
    unsigned n = gl_param_count(pname);
    memset(out, 0, sizeof(out));
    if (tspgl_es1_get_booleanv(pname, params))
        return;
                  ...
                  const void *pixels)
{
    ...
    /* Presenter drops mips (MAX_LEVEL 0). Skip the extra IPC. */
    if (level != 0)
        return;
                           ...
                           const void *ptr)
{
    ...
    if (nlog < 8) {
        fprintf(stderr,
                "tspgl: AttribPtr i=%u size=%d type=0x%x stride=%d ptr=%p vbo=%u off=%d\n",
                index, (int)size, type, (int)stride, ptr, bound_array,
                ptr_is_vbo_offset(ptr));
        fflush(stderr);
        nlog++;
    }
    attribs[index].size = size;
...
{
    unsigned i;
    static uint8_t scratch[512u * 1024u];
    /* A bound VAO must not skip client-memory attribs: MEI/loader mix VAO
     * with stack pointers. Skip only attribs that are true VBO offsets. */
    for (i = 0; i < 16; ++i) {
        ...
        if (attribs[i].type == 0x140C && src) { /* GL_FIXED 16.16 */
            unsigned v, c, sz = (unsigned)attribs[i].size;
            uint8_t *dst = buf + 16;
            for (v = 0; v < (unsigned)count; ++v) {
                int32_t *fx = (int32_t *)(dst + v * stride);
                float *fl = (float *)fx;
                for (c = 0; c < sz; ++c)
                    fl[c] = (float)fx[c] / 65536.f;
            }
        }
...
/* M3G often leaves TEXTURE_COORD_ARRAY off while TexCoordPointer + a 2D
 * bind are live (env-map / lighting overlays). Those draws hit the shader
 * as untextured white. Upload UVs for this draw only, then restore. */
static int force_uv_on;
static int force_uv_from_a1;
static int force_tex2d_on;
static int pixel_ortho_on;
static int gles2_mat_on;

static float tspgl_fabs(float v)
{
    return v < 0.f ? -v : v;
}

static int any_2d_bound(void)
{
    unsigned u;
    for (u = 0; u < 32; ++u) {
        if (tex_bind_2d[u])
...
static uint32_t bound_2d_tex(void)
{
    return tex_bind_2d[active_tex < 32 ? active_tex : 0];
}

static void begin_force_uv(void)
{
    force_uv_on = 0;
    force_uv_from_a1 = 0;
    /* GLES2 interleaved pos+uv: attrib 1 is vec2 UV, not GLES1 color.
     * The ES1 shader samples UVs at location 3. Server UploadAttrib uses
     * sattr[index] from VertexAttribPointer — must send pointer for 3. */
    if (!attribs[3].enabled && attribs[1].enabled && attribs[1].size == 2 &&
        attribs[1].ptr) {
        tspgl_es1_set_pointer(3, attribs[1].size, attribs[1].type,
                              attribs[1].normalized, attribs[1].stride,
                              (const void *)(uintptr_t)attribs[1].ptr);
        attribs[1].enabled = 0;
        tspgl_es1_enable_array(3, 1);
        tspgl_es1_enable_array(1, 0);
        force_uv_on = 1;
        force_uv_from_a1 = 1;
        return;
    }
    if (attribs[3].enabled || attribs[3].is_offset)
        return;
    if (attribs[3].size <= 0 || attribs[3].ptr == 0)
        return;
    /* Env-map leftovers are size 3. Forcing those UVs onto 2D VFX (cutter
     * beam) stretches the sphere-map across the screen. */
    if (attribs[3].size >= 3)
        return;
    if (!any_2d_bound())
        return;
    tspgl_es1_set_pointer(3, attribs[3].size, attribs[3].type,
                          attribs[3].normalized, attribs[3].stride,
                          (const void *)(uintptr_t)attribs[3].ptr);
    tspgl_es1_enable_array(3, 1);
    force_uv_on = 1;
}

static void end_force_uv(void)
{
    if (!force_uv_on)
        return;
    if (force_uv_from_a1) {
        attribs[1].enabled = 1;
        attribs[3].enabled = 0;
        tspgl_es1_enable_array(1, 1);
        tspgl_es1_enable_array(3, 0);
        force_uv_from_a1 = 0;
        force_uv_on = 0;
        return;
    }
...
static void begin_force_tex(void)
{
    force_tex2d_on = 0;
    if (!any_2d_bound())
        return;
    if (tspgl_es1_is_enabled(GL_TEXTURE_2D) == 1)
        return;
    tspgl_es1_set_enable(GL_TEXTURE_2D, 1);
    force_tex2d_on = 1;
}

static void end_force_tex(void)
{
    if (!force_tex2d_on)
        return;
    tspgl_es1_set_enable(GL_TEXTURE_2D, 0);
    force_tex2d_on = 0;
}

static void begin_pixel_ortho(GLint first, GLsizei count)
{
    unsigned stride, i, n;
    const uint8_t *src;
    float maxa, maxz, x0, y0, z0, u0, v0;
    int vpw, vph, screen2d;
    static int nlog;

    pixel_ortho_on = 0;
    gles2_mat_on = 0;
    if (count <= 0 || !attribs[0].enabled || attribs[0].is_offset)
        return;
    if (attribs[0].type != GL_FLOAT || attribs[0].size < 2 || !attribs[0].ptr)
        return;
    stride = attribs[0].stride
                 ? (unsigned)attribs[0].stride
                 : (unsigned)attribs[0].size * 4u;
    if (!stride)
        return;
    src = (const uint8_t *)(uintptr_t)attribs[0].ptr;
    n = (unsigned)count;
    maxa = 0.f;
    maxz = 0.f;
    x0 = y0 = z0 = u0 = v0 = 0.f;
    vpw = (int)st_viewport[2];
    vph = (int)st_viewport[3];
    if (vpw <= 0)
        vpw = 640;
    if (vph <= 0)
        vph = 480;
    screen2d = 1;
    for (i = 0; i < n; ++i) {
        const float *p = (const float *)(src + (unsigned)first * stride +
                                         i * stride);
        float ax = tspgl_fabs(p[0]);
        float ay = tspgl_fabs(p[1]);
        float az = attribs[0].size >= 3 ? tspgl_fabs(p[2]) : 0.f;
        if (i == 0) {
            x0 = p[0];
            y0 = p[1];
            z0 = attribs[0].size >= 3 ? p[2] : 0.f;
        }
        if (ax > maxa)
            maxa = ax;
        if (ay > maxa)
        ...
        if (az > maxz)
            maxz = az;
        if (p[0] < -8.f || p[1] < -8.f || p[0] > (float)vpw + 8.f ||
            p[1] > (float)vph + 8.f)
            screen2d = 0;
    }
    if (maxz > 2.5f)
        screen2d = 0;
    if (attribs[3].enabled && attribs[3].ptr && attribs[3].type == GL_FLOAT &&
        attribs[3].size >= 2 && !attribs[3].is_offset) {
        unsigned us = attribs[3].stride
                          ? (unsigned)attribs[3].stride
                          : (unsigned)attribs[3].size * 4u;
        const float *uv = (const float *)(uintptr_t)(attribs[3].ptr +
                                                     (uint32_t)first * us);
        u0 = uv[0];
        v0 = uv[1];
    }
    if (nlog < 12) {
        fprintf(stderr,
                "tspgl: vert0 x=%g y=%g z=%g uv=%g,%g maxxy=%g maxz=%g n=%u tex=%u vp=%dx%d a3=%d mvp=%d screen2d=%d\n",
                x0, y0, z0, u0, v0, maxa, maxz, n, bound_2d_tex(),
                vpw, vph, attribs[3].size, (int)gles2_mvp_valid, screen2d);
        fflush(stderr);
        nlog++;
    }
    /* Prefer the engine's GLES2 MVP for 3D / transformed UI.
     * Do not apply a leftover 3D matrix onto NDC 2D quads (splash/HUD). */
    if (gles2_mvp_valid && !(maxa <= 2.5f && maxz <= 2.5f)) {
        glMatrixMode(GL_PROJECTION);
        glPushMatrix();
        glLoadMatrixf(gles2_mvp);
    ...
    /* Fallback: only screen-pixel 2D (HUD in pixels), never world meshes. */
    if (!screen2d || maxa <= 2.5f)
        return;
    glMatrixMode(GL_PROJECTION);
    glPushMatrix();
    glLoadIdentity();
    glOrthof(0.f, (float)vpw, (float)vph, 0.f, -1.f, 1.f);
    glMatrixMode(GL_MODELVIEW);
    pixel_ortho_on = 1;
}

static void end_pixel_ortho(void)
{
    if (gles2_mat_on) {
        glMatrixMode(GL_MODELVIEW);
        glPopMatrix();
...
static void begin_mei_draw(GLint first, GLsizei count)
{
    /* GLES2 shaders own attrib 1 as UV and their own MVP. Do not remap
     * to the ES1 layout or inject ortho — that flattened the world and
     * blew out menu lighting. */
    if (cur_prog)
        return;
    begin_force_uv();
    begin_force_tex();
    begin_pixel_ortho(first, count);
}

static void end_mei_draw(void)
{
    end_pixel_ortho();
    end_force_tex();
    end_force_uv();
...
void glDrawArrays(GLenum mode, GLint first, GLsizei count)
{
    ...
    static int nlog;
    if (nlog < 6) {
        fprintf(stderr, "tspgl: DrawArrays mode=0x%x first=%d count=%d vp=%dx%d\n",
                mode, (int)first, (int)count, (int)st_viewport[2],
                (int)st_viewport[3]);
        fflush(stderr);
        nlog++;
    }
    begin_mei_draw(first, count);
...
void glDrawElements(GLenum mode, GLsizei count, GLenum type, const void *idx)
{
    ...
    int client_idx = idx && (uintptr_t)idx >= 0x10000u;
    int indexed = (bound_element || bound_vao) && !client_idx;
    static int nlog;
    if (nlog < 8) {
        fprintf(stderr,
                "tspgl: DrawElements mode=0x%x count=%d type=0x%x idx=%p fb=%u vp=%dx%d vao=%u vbo=%u ebo=%u a0=%d/%p/%d a1=%d/%p/%d\n",
                mode, (int)count, type, idx, bound_fb, (int)st_viewport[2],
                (int)st_viewport[3], bound_vao, bound_array, bound_element,
                attribs[0].size, (void *)(uintptr_t)attribs[0].ptr,
                attribs[0].is_offset, attribs[1].size,
                (void *)(uintptr_t)attribs[1].ptr, attribs[1].is_offset);
        fflush(stderr);
        nlog++;
    }
    unsigned nb = indexed ? 0 : (unsigned)count * esize;
    uint8_t *buf;
    static uint8_t scratch[256u * 1024u];
    int heap = 0;
    unsigned verts = (unsigned)count;
    if (!indexed && idx)
        verts = max_index_value(count, type, idx) + 1u;
    begin_mei_draw(0, (GLsizei)verts);
...
static int mat4_looks_mvp(const float *m)
{
    /* Perspective / clip: w-row carries -1, m[15] ~ 0. */
    if (tspgl_fabs(m[11]) > 0.1f && tspgl_fabs(m[15]) < 0.25f)
        return 1;
    /* 2D ortho: last row 0,0,0,1 and tiny scale (2/width). */
    if (tspgl_fabs(m[15] - 1.f) < 0.02f && tspgl_fabs(m[3]) < 0.02f &&
        tspgl_fabs(m[7]) < 0.02f && tspgl_fabs(m[11]) < 0.02f &&
        tspgl_fabs(m[0]) < 0.05f && tspgl_fabs(m[5]) < 0.05f)
        return 1;
    return 0;
}

static void capture_gles2_mvp(GLint loc, GLsizei count, GLboolean tr,
                              const GLfloat *v)
{
    float tmp[16];
    int r, c;
    ...
    if (tr) {
        ...
        memcpy(tmp, v, 64);
    for (i = 0; i < LOC_N; ++i) {
        if (ulocs[i].prog == cur_prog && ulocs[i].loc == loc) {
            nm = ulocs[i].name;
            break;
        }
    }
    if (nm) {
        ...
        if (hit) {
            memcpy(gles2_mvp, tmp, 64);
            gles2_mvp_valid = 1;
            if (nlog < 8) {
                fprintf(stderr, "tspgl: gles2 mvp loc=%d %s m11=%g m15=%g\n",
                        (int)loc, nm, tmp[11], tmp[15]);
                fflush(stderr);
                nlog++;
            }
            return;
    ...
    if (!mat4_looks_mvp(tmp))
        return;
    memcpy(gles2_mvp, tmp, 64);
    gles2_mvp_valid = 1;
    if (nlog < 8) {
        fprintf(stderr, "tspgl: gles2 mvp loc=%d heur m0=%g m5=%g m11=%g m15=%g\n",
                (int)loc, tmp[0], tmp[5], tmp[11], tmp[15]);
        fflush(stderr);
        nlog++;
    }
}
...
{
    static unsigned nswap;
    (void)dpy;
    (void)surf;
    nswap++;
    if (nswap == 1 || (nswap % 60u) == 0) {
        fprintf(stderr, "tspgl: SwapBuffers n=%u fb=%u vp=%dx%d\n", nswap,
                bound_fb, (int)st_viewport[2], (int)st_viewport[3]);
        fflush(stderr);
    }
...
{
    tspgl_shared();
    fprintf(stderr, "tspgl: 32-bit GLES1 client loaded\n");
}
